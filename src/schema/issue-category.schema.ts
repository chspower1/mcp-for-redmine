...
export const RedmineIssueCategorySchema = z.object({
  ...
  assigned_to: RedmineReferenceSchema.optional().describe(
    "Optional user automatically assigned to issues in this category"
  ),
});
...
const IssueCategoryRequestObjectSchema = z.object({
  ...
  assigned_to_id: z
    .number()
    .optional()
    .describe("User ID to automatically assign issues in this category"),
});
...
export const GetIssueCategoryToolSchema = z
  .object({
    id: z
      .number()
      .describe("The numeric ID of the issue category to retrieve detailed information for"),
  })
...
export const UpdateIssueCategoryToolSchema = UpdateIssueCategoryRequestSchema.shape.issue_category
  .extend({
    id: z.number().describe("The numeric ID of the issue category to update"),
  })
  .describe("Update an existing issue category's name or assigned user configuration");

export const DeleteIssueCategoryToolSchema = z
  .object({
    id: z.number().describe("The numeric ID of the issue category to delete"),
    reassign_to_id: z
      .number()
      .optional()
      .describe("Optional category ID to reassign existing issues to before deletion"),
  })
